...
const CommonBarChart = ({
  ...
  useEffect(() => {
    ...
    // Axis Domains
    xAxis.domain(data && data.map(d => d.brandName));
    yAxis.domain([0, d3.max(data, d => d.quantity) + 10]);
    ...
    xAxisGroup.transition(trans).call(
      d3.axisBottom(xAxis).tickFormat(d => {
        const dataObj = data.filter(da => da.brandName === d);
    ...
    barGroups
      ...
      .attr('transform', d => `translate(${xAxis(d.brandName)},0)`);
